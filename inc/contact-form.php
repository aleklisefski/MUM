...
// That is, https://www.mum.edu/about-mum/location-and-contact/contact-us/
...
<form action="https://www.mum.edu/thank-you/" method="post" name="contactForm" id="contact-form" class="cuformvalidate">
...
<input type="radio" name="usCitizen" value="US" id="US"><label for="US">Yes</label>
<input type="radio" name="usCitizen" value="Other" id="Other"<?php if ($this_is_intl_cu_form) {echo ' checked';} ?>><label for="Other">No</label><span id="cuintl_redirecting_msg"></span>
			</span>
	...
	<div class="item third">
		...
		<p for="opt-in">to receive MUM email updates</p>
		</div>
	</div>
	<div class="clearFix"></div>
...
jQuery('#US').on('click', function(){
	if ($international_required) {
		jQuery('#cuintl_redirecting_msg').text('Redirecting...');
		top.location.href='https://www.mum.edu/about-mum/location/contact-us/?cusource_of_redirect=' + document.contactForm.CUSOURCE.value;
	}
	else {
		jQuery('#cuintl_redirecting_msg').text('');
	}
});
...
jQuery('#Other').on('click', function(){
	if (!$international_required) {
		jQuery('#cuintl_redirecting_msg').text('Redirecting...');
		top.location.href='https://www.mum.edu/admissions/international-students/contact-us-intl/?cusource_of_redirect=' + document.contactForm.CUSOURCE.value;
	}
	else {
		jQuery('#cuintl_redirecting_msg').text('');
	}
...
jQuery('#cu_submit_anchor').on('click', function(){
	if( jQuery('#contact-form').valid() ) {
		jQuery('#cu_sending_msg').show();
	}
	else {
		jQuery('#cu_sending_msg').hide();
		return false;
	}
});
