<?php
	/*
	Template Name: Department - David Lynch MA TEST
	*/
...
<?php
	...
	$video_title_overlay_3 = get_field('video_title_overlay_3');
	
	$letter_photo = get_field('letter_photo');
	$letter_content = get_field('letter_content');
	$letter_signature = get_field('letter_signature');
?>
        ...
        <div class="slide">
            ...
            <div class="video-overlay options">
	            <?php if( !empty($video_title_overlay_1) ) { ?>
	            	<h1 class="small"><?php echo $video_title_overlay_1; ?></h1>
	            <?php } ?>
	            <?php if( !empty($video_title_overlay_2) ) { ?>
	            	<h2 class="large"><span><?php echo $video_title_overlay_2; ?></span></h2>
	            <?php } ?>
	            <?php if( !empty($video_title_overlay_3) ) { ?>
	            	<div class="name"><?php echo $video_title_overlay_3; ?></div>
	            <?php } ?>
            </div>
            
            <div class="slide-info download blue options">
                <div class="heading blue border-top">
                  <h4><?php echo $form_heading; ?></h4>
					<?php if(get_field('interest_select_options')): $option_count = 1 ?>
						<form>
							<div class="select">
								<select id="interest_options">
									...
									?>
									<option value="<?php echo '#popup-' . $option_count ?>"><?php echo $select_option_label; ?></option>
					 				<?php $option_count ++; endwhile; ?>
					 			</select>
							</div>
							<a class="button fancybox-content" id="get_started_button" href="#popup-1"><?php if( $download_button_text ) { echo $download_button_text; } else { echo 'Get Started'; } ?></a>
...
$(function() {
    ...
    $('#interest_options').change(function() {
    	var popup_id = $(this).val();
        $('#get_started_button').attr('href', popup_id)
    });
});
...
<section id="lynch-letter" class="dark">
 	<div class="gradiant"></div>
 	<div class="container">	
		<div class="col width-60">
			<?php if( !empty($letter_content) ) { echo $letter_content; } ?>
			<?php if( !empty($letter_signature) ) { ?>	
				<img class="sig" src="<?php echo $letter_signature; ?>" alt="David Lynch" />
				<div class="clearFix"></div>
			<?php } ?>
		</div>
		<div class="col width-40">
			<?php if( !empty($letter_photo) ) { ?>
				<img src="<?php echo $letter_photo; ?>" alt="David Lynch" />
			<?php } ?>
		</div>
 	</div>	
</section>
...
<section id="page-header" class="tan">
 	<div class="container">	
		...
		<aside>
			<!-- Custom Sub-nav to show DMLA child pages even when this is not parent -->
			<!-- #################################################################### -->
			<!-- #################################################################### -->
			...
			<?php
				...
				$children = get_pages(array('child_of' => 31419));
				$childCount = count($children);
				$parent = array_reverse(get_post_ancestors(31419));
				$parentCount = count($parent);		
		
				$top_parent_name = 'Home';
				$top_parent_link = home_url();
				$second_parent = get_post($post);
				$second_parent_id = 31419;
			...
			<!-- Above to be replaced by subnav include on all but this A/B test page -->
			<!-- #################################################################### -->
			<!-- #################################################################### -->
...
<?php
	$mailchimp_form_u = get_field('mailchimp_form_u');
	$mailchimp_form_id = get_field('mailchimp_form_id');
	$fb_tracking_label = get_field('fb_tracking_label');
?>
...
<?php if(get_field('interest_select_options')): $option_count = 1 ?>
	<?php while(has_sub_field('interest_select_options')): ?>
		...
		<div id="<?php echo 'popup-' . $option_count ?>" class="lynch-signup hidden">
			<div class="fish"></div>
			...
			<div id="mc_embed_signup">
				<form action="//mum.us8.list-manage.com/subscribe/post?u=22fad1e3d15e93d858349d4a6&amp;id=<?php echo $mailchimp_form_id; ?>" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" class="validate" target="_blank" novalidate>
					...
					<div id="mc_embed_signup_scroll">
						...
						<div style="position: absolute; left: -5000px;"><input type="text" name="b_22fad1e3d15e93d858349d4a6_<?php echo $mailchimp_form_id; ?>" tabindex="-1" value=""></div>
						<div class="clear"><input type="submit" value="Download Now!" name="subscribe" id="mc-embedded-subscribe" class="button" onclick="fbq('track', '<?php echo $fb_tracking_label . ' - ' . $select_option_label; ?>');"></div>
					</div>
				</form>
			</div>
			<!--End mc_embed_signup-->
